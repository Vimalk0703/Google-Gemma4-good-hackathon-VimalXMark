...
Kaggle's Project Link description field has a **200-character limit**. The seven blocks below are paste-ready — each description is plain text, kept under the cap with a few characters of headroom in case Kaggle counts trailing whitespace.
...
```
Open clinical portal, no login. Click "Load sample audio" to run the full Tier-1 pipeline — fine-tuned Gemma 4 + AI Clinical Note — on a real ICBHI clip in ~7 seconds. The five-minute proof.
```
...
```
Apache 2.0 end-to-end — Flutter app, Python service, fine-tuning notebooks, clinic server, web portal. Start at README "Evidence at a glance" — every claim mapped to a file you can verify.
```
...
```
Unsloth-track submission. Gemma 4 E4B + QLoRA on ICBHI 2017, patient-level held-out split, 85% crackle detection on patients the model never heard. Reproducible from notebook 06.
```
...
```
The full 12-minute arc — problem, Umlazi mother, why Gemma 4, the closing argument on open source. Every claim cited in SOURCES.md. Must-watch for the full picture behind the 3-minute pitch.
```
...
```
Tier-0 proof. Unedited walkthrough of the Flutter app on a real $60 Samsung A53 in airplane mode — Gemma 4 E2B fully offline. Voice, camera, vision, IMCI classification, treatment plan. No cuts.
```
...
```
Primary-source URL for every claim — UNICEF, WHO, Cochrane, GSMA, peer-reviewed journals, ICBHI 2017. Fact-check any number in the writeup or videos in under a minute.
```
...
```
The capabilities Malaika deliberately does NOT claim — GPU constraints, hackathon-grade vs FDA-cleared, single-encounter, and "projection, not measurement" on the 175K-children-a-year number.
```
