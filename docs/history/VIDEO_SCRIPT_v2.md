...
*~13:30 total (with the two pre-recorded demo clips inside §10 and §11).*
*Pause where it says (pause). Breathe where there's a blank line.*
...
## 6. WHY PNEUMONIA  *(3:20 – 4:00)*  · **VIMAL**
...
## 7. THE MODEL CHOICE  *(4:00 – 4:55)*  · **MARK**
...
## 8. THE ARCHITECTURE — BELIEF STATE & SKILLS  *(4:50 – 5:45)*  · **MARK**
...
## 9. WHY FINE-TUNE — AND WHY *NOT*  *(5:45 – 6:40)*  · **MARK**
...
## 10. THE PHONE — WHAT WE BUILT, AND WHAT IT DOES  *(6:40 – 8:50)*  · **MARK**

So — what *is* Malaika, on the phone?

It is a single application, sitting on a sixty-dollar Android, that runs the entire WHO IMCI assessment without ever touching a network.

Inside it, **Gemma 4 E2B** does four jobs that together replace what a WHO-trained nurse does in the consulting room.

**It listens.** Offline speech-to-text on the Android native engine. The mother speaks Swahili, Hausa, Hindi, Bengali, Yoruba, Amharic, Portuguese — Gemma understands her, in her language, with no translation pipeline.

**It looks.** One photo from the gallery. Six vision skills, each one a typed Gemma 4 tool call: alertness, sunken eyes, chest indrawing, dehydration, wasting, edema.

**It reasons.** The twelve-skill agent we just talked about. Belief state, contradiction detection, danger-sign halt logic — all running on the phone's GPU at about fifty tokens a second.

**It speaks.** Offline text-to-speech on the Android native engine. The treatment plan read back in the mother's language, on the phone speaker.

All four run entirely on-device. No SIM. No Wi-Fi. No cloud. Ever.

(half-beat)

Now look at a real assessment, in the Malaika app — on a real Samsung A53, in airplane mode. One continuous take. Nothing was cut. Nothing was faked.

*[—— PRE-RECORDED PHONE WALKTHROUGH PLAYS HERE — ~75–90 seconds — full audio retained — sourced from `DEMO_WALKTHROUGH.md` ——]*

*(Camera holds the demo full-screen. Vimal and Mark are off-camera, silent, while the demo plays its own narration.)*

(half-beat — return to two-presenter shot)

What you just watched is the part of Malaika that **is** AI.

The red classification card at the end is the part where Malaika **stops being AI.**

Every classification on that screen is a hard-coded WHO IMCI threshold, written into Dart, with the chart-book page cited above the line. Not a probability. Not a model's guess. Not a hallucination.
...
## 11. THE OTHER APP — THE CLINIC PORTAL  *(8:50 – 11:00)*  · **MARK**

Two surfaces. Two users. Two designs.

The Android app you just watched is for **the mother.** Fully offline. In her pocket. In her language. Built for a sixty-dollar phone in a village with no signal.
...
So in the village clinic — ten kilometres away, with a three-hundred-dollar mini-PC — we run **Gemma 4 E4B** plus the LoRA adapter we just talked about. And in front of it, a browser-based clinical portal that any nurse can sign into from any laptop the clinic already owns.

What that portal does is built around three things that are easy to claim and hard to actually ship —

**A live connection-health banner.** The nurse knows, *before* she records, whether the clinic server is up. Green is up. Yellow is degraded. Red is down. No silent failures.

**Browser-native audio recording.** The nurse can drop a `.wav` from disk, *or* press a record button and capture thirty seconds of breath sounds in the browser. No app to install. No SDK. The audio is encoded into PCM 16-bit WAV in-browser, then sent to the clinic server.

**The AI Clinical Note.** This is the defining feature of the clinic tier. After the auscultation classifier returns its label and confidence, the **same Gemma 4 model runs a second pass** with a "senior nurse mentoring a junior colleague" prompt. Out comes a 3–4 sentence clinical reasoning note for the chart — grounded in WHO IMCI, naming the next protocol step, ending with a specific recommendation. *That* is the difference between an AI tool and an AI clinician colleague.

(half-beat)

Same hand-off as before. Look at the live portal.

*[—— PRE-RECORDED WEB PORTAL WALKTHROUGH PLAYS HERE — ~60–75 seconds — full audio retained — recorded from the live deployed site ——]*

> **What's on screen during this clip, in order — the walkthrough Vimal records:**
>
> 1. **Landing page** — IBM Plex typography, brand mark, the two-tier architecture diagram. *"This is the marketing surface; the portal is one click in."*
> 2. **Sign-in** — the passcode field; entering the shared clinic passcode.
> 3. **Connection-health banner** — visible green dot at the top right: *"clinic server: up."*
> 4. **Two input modes** — the nurse can drop a `.wav` *or* press record. The record button is shown first.
> 5. **Live recording in the browser** — fifteen seconds of breath audio captured. Waveform animates. Stop. Encodes to WAV in-browser.
> 6. **Multi-stage progress** — `upload → spectrogram → inference`. Each stage lights up in turn.
> 7. **Result card** — classification (`abnormal`), confidence (`91%`), and the mel-spectrogram thumbnail.
> 8. **The AI Clinical Note** — the senior-nurse-voice paragraph reads out on screen, three to four sentences, grounded in WHO IMCI.
> 9. **WHO IMCI context block** — *"per protocol: count respiratory rate over 60 seconds, examine for chest indrawing, refer urgently if any danger sign is present."*

*(Camera holds the portal full-screen. Vimal and Mark are off-camera, silent, while the demo plays its own narration.)*

(half-beat — return to two-presenter shot)

No app to install. No SDK. No cloud. The audio file goes from the nurse's browser to the clinic's own server, and back.

**Same Gemma family. Same open-weights story.** Same on-device privacy — the clinic server is the *clinic's own* hardware, never anyone else's cloud.
...
## 12. THE STORY, AGAIN  *(11:00 – 11:50)*  · **VIMAL**
...
## 13. WHAT THIS MEANS  *(11:50 – 12:10)*  · **VIMAL**
...
## 14. THE CLOSE  *(12:10 – 13:30)*  · **VIMAL** *(M stands beside, silent)*
...
| Speaker / Source | Sections | Approx. time | Share |
|------------------|----------|-------------:|------:|
| **Vimal** (live) | 1, 2, 3, 4, 5, 6, 12, 13, 14 | ~7:15 | 54% |
| **Mark** (live) | 7, 8, 9, 10a + 10b, 11a + 11b | ~3:30 | 26% |
| **Pre-recorded demo clips** | 10 (phone, ~90s) + 11 (web portal, ~70s) | ~2:45 | 20% |

Vimal carries the human arc — open, problem, story, why-pneumonia, redemption, close.
Mark carries the engineering arc — model choice, belief-state-and-skills, fine-tuning principle, then sets up and lands each pre-recorded demo (phone, then web portal).
...
- **§10 and §11 each contain a pre-recorded demo clip.** When the clip starts, cut camera away from the two presenters and let the clip play full-screen with its own audio. Vimal and Mark stay silent and out of frame for the entire clip. When it ends, cut back to the two-presenter shot for the closing beat.
- **The phone demo clip** is sourced from `DEMO_WALKTHROUGH.md`. Use a 75–90s cut of the full 3:30 walkthrough — keep airplane-mode bookends, voice greeting, Q&A → vision → classification → treatment. Drop anything that doesn't earn its second.
- **The web portal demo clip** is recorded from the live deployed portal. Hit every beat in the §11 numbered list — landing → passcode → connection-health → record-in-browser → multi-stage progress → result card → AI Clinical Note → WHO IMCI context. ~60–75s.
...
- **The "Open weights are how that lever gets out of the data centre" line in §14 is the philosophical core.** Deliver it slowly. This is the line a Google judge will quote when they nominate the project.
