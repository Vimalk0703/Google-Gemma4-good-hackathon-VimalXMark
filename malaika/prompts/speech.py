...
# --- Understand Caregiver's Spoken Response (original — kept for future native audio support) ---
UNDERSTAND_RESPONSE = PromptRegistry.register(
    PromptTemplate(
...
# --- Understand Caregiver's Response from Text (Whisper transcription → Gemma 4 reasoning) ---
UNDERSTAND_RESPONSE_FROM_TEXT = PromptRegistry.register(
    PromptTemplate(
        name="speech.understand_response_from_text",
        version="1.0.0",
        description=(
            "Understand a caregiver's response from transcribed text "
            "(produced by Whisper), extracting intent, clinical entities, "
            "and yes/no answer. Used when native audio input is unavailable."
        ),
        system_prompt=SYSTEM_SPEECH_LISTENER,
        user_template=(
            'Malaika asked the caregiver: "{question_asked}"\n\n'
            "The caregiver's spoken response was transcribed by a speech recognition system:\n\n"
            "--- TRANSCRIPTION ---\n"
            "{transcription}\n"
            "--- END TRANSCRIPTION ---\n\n"
            "Extract the following from the transcribed response:\n"
            "1. INTENT: What is the caregiver trying to communicate?\n"
            "   - 'affirmative' = yes, agrees, confirms\n"
            "   - 'negative' = no, denies, disagrees\n"
            "   - 'informative' = providing information/details\n"
            ...
            "   - 'unrelated' = response does not address the question\n\n"
            "2. ENTITIES: Any clinical facts mentioned (symptoms, durations, quantities).\n\n"
            "3. LANGUAGE: What language was the transcription in? (The transcriber may have "
        ...
        required_variables=frozenset({"question_asked", "transcription"}),
        expected_output_format="json",
        output_schema={
            "type": "object",
            "properties": {
